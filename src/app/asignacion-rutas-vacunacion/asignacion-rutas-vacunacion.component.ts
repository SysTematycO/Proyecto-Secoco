...
@Component({
  selector: 'app-asignacion-rutas-vacunacion',
  templateUrl: './asignacion-rutas-vacunacion.component.html',
  styleUrls: ['./asignacion-rutas-vacunacion.component.css']
})
export class AsignacionRutasVacunacionComponent implements OnInit {
  ...
  async servicio() {
    ...
    await this.httpClient.get("http://localhost:8081/competitors/rutasVacunacion").subscribe(data => {
      datos = data[0]
      ...
      for(let i in data){
        ...
        if(Number(i)%3==0){
          ...
          const titulo = this.renderer.createText('Cita de vacunación');
