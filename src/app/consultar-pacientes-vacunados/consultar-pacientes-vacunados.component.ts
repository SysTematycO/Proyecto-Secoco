...
@Component({
  selector: 'app-consultar-pacientes-vacunados',
  templateUrl: './consultar-pacientes-vacunados.component.html',
  styleUrls: ['./consultar-pacientes-vacunados.component.css']
})
export class ConsultarPacientesVacunadosComponent implements OnInit {
  ...
  async servicio() {
    ...
    await this.httpClient.get("http://localhost:8081/competitors/pacientesVacunadosPorFecha/"+fecha).subscribe(data => {
      datos = data[0]
      ...
      for(let i in data){
        ...
        if(Number(i)%3==0){
          ...
          const text05 = this.renderer.createText('Vacuna: '+ data[i].vacuna);
          this.renderer.appendChild(cardText05,text05);
          this.renderer.appendChild(cardBody,cardText05);

          var cardText06 = this.renderer.createElement("p");
          this.renderer.setAttribute(cardText06,"class","card-text");
          const text06 = this.renderer.createText('Dosis: '+ data[i].dosis);
          this.renderer.appendChild(cardText06,text06);
          this.renderer.appendChild(cardBody,cardText06);
      }
