...
@Component({
  selector: 'app-pacientes-sin-vacunar',
  templateUrl: './pacientes-sin-vacunar.component.html',
  styleUrls: ['./pacientes-sin-vacunar.component.css']
})
export class PacientesSinVacunarComponent implements OnInit {
  ...
  async servicio() {
    ...
    await this.httpClient.get("http://localhost:8081/competitors/pacientesSinVacunar").subscribe(data => {
      datos = data[0]
      ...
      for(let i in data){
        ...
        if(Number(i)%3==0){
          ...
          const titulo = this.renderer.createText('Paciente');
